...
float PIDController::clamp(float input, float min_value, float max_value)
{
...
float PIDController::update(float setpoint, float actual, float dt);
{
    ...
    pid_.integral += pimp.error *pid_.sample_time;
    ...
    float derivative_kick_factor = (pid_.current_effort - pid_.last_position) /pid_.sample_time; 
    ...
    pid_.last_position = pid_.current_effort;
