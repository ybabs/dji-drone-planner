...
Planner::Planner()
{
   ...
   error_publisher = nh.advertise<std_msgs::Float32>("a3/landing_error", 10);
...
void Planner::onWaypointReached()
{
   ...
   if (!flight_plan.empty())
   {
      if (checkTask == 1)
      {
         ...
         if (initiate_record)
         {
            std_msgs::UInt8 record_command;
            record_command.data = 1;
            getPositionError(current_gps_location, flight_plan[waypoint_index]);
            daqrecord_publisher.publish(record_command);
            // start recording stuff here
            ros::Duration(recording_time).sleep();
            // send command to stop recording data
            record_command.data = 2;
            daqrecord_publisher.publish(record_command);
...
void Planner::computeLandingError()
{
    ...
    ROS_INFO("Average landing error for mission at %f m/s speed is %f", uav_speed, avg_error);
}
...
double Planner::getPositionError(sensor_msgs::NavSatFix &start_coord, gcs::Waypoint &end_coord)
{
